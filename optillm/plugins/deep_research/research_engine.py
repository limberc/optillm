...
from optillm.plugins.deep_research.session_state import close_session, get_session_manager
...
def clean_reasoning_tags(text: str) -> str:
    ...
    cleaned_text = re.sub(r'  +', ' ',
                          cleaned_text)  # Multiple spaces to single space (but preserve intentional double spaces)
...
def validate_citation_usage(text: str, total_citations: int) -> Dict[str, Any]:
    ...
    if usage_percentage < 30:
        result[
            "warning"] = f"Very low citation usage ({usage_percentage:.1f}%). Most sources are not cited in the text."
...
def validate_report_completeness(text: str) -> Dict[str, Any]:
    ...
    for section_title, section_content in sections:
        ...
        placeholder_count = len(
            re.findall(r'\[[^\]]*(?:placeholder|needed|research|todo|tbd)[^\]]*\]', section_content, re.IGNORECASE))
